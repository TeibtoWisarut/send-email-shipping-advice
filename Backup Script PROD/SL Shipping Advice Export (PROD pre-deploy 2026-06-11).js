    ...
    */
    ...
    function Page3_PreviewData(context, params) {
        ...
        /*
            ...
            billaddress:""
            billaddressee:" "
            billingaddress:"33681"
            billingaddress_key:"33681"
    
            shipaddr1:" "
            shipaddress:""
            shipaddressee:" "
            shippingaddress:"33681"
            shippingaddress_key:"33681"
        */
        // let recRecordObj = libUtility.getLookupFields('salesorder', pi_internalid, ['billingaddress_key', 'shippingaddress_key']);
        // ดึงข้อมูล Container
        ...
        const maxContainerIndex = containerList.length > 0
            ...
            : 0;
        defaultFieldValue.custbody_total_container_text = libUtility.jsonStringify(containerList);
    ...
    function Page4_SubmitData(context, params) {
        ...
        try {
            ...
            /**
             *
            let recRecordPI = record.load({
                type: 'salesorder',
                id: so_id_main,
                isDynamic: true,
            });
    
            log.debug('recRecordPI', recRecordPI);
    
            let lineCountConCal = recRecordPI.getLineCount({ sublistId: 'recmachcustrecord_concal_ref_so' });
            for (var i = lineCountConCal - 1; i >= 0; i--) {
                let item_id = recRecordPI.getSublistValue({ sublistId: 'item', fieldId: 'custrecord_concal_item', line: i }); // Item
                let so_line = recRecordPI.getSublistValue({ sublistId: 'item', fieldId: 'custrecord_concal_ref_line_so', line: i }); // Item
    
                let findValue = resultsItemDetails.find(item => Number(item.item_id) === Number(item_id) && Number(item.line_id) === Number(so_line));
                if (findValue) {
                    recRecordPI.selectLine({ sublistId: 'item', line: i });
                    recRecordPI.setCurrentSublistValue({ sublistId: 'item', fieldId: 'custrecord_concal_ref_sa_no', value: findValue.line_id || '' });
                    recRecordPI.setCurrentSublistValue({ sublistId: 'item', fieldId: 'custrecord_concal_ref_sa_lineid', value: findValue.line_id || '' });
                    recRecordPI.commitLine({ sublistId: 'item' });
                }
            }
             */
            // ################################################################################################
        ...
        function getEmailData(recid) {
            ...
            try {
                data = {
                    ...
                    vgm_cutoff_time:          so.getText('custbody_ex_vgmcutoff_time') || so.getValue('custbody_ex_vgmcutoff_time') || '',
                    items: []
                };
            ...
            try {
                ...
                for (let i = 0; i < lineCount; i++) {
                    ...
                    if (itemType === 'Discount' || itemType === 'Subtotal') continue;
            ...
            // ----- 3a.4 Batch lookup Paper Size from Item Record -----
                ...
                for (let k = 0; k < itemIdsForLookup.length; k++) {
                    ...
                    try {
                        const itemLookup = search.lookupFields({
                            type: 'item',
                            id: iid,
                            columns: ['custitem_infor_paper_size']
                        });
                        let psText = '';
                        const ps = itemLookup.custitem_infor_paper_size;
                        if (Array.isArray(ps) && ps.length > 0) {
                            psText = ps[0].text || ps[0].value || '';
                        } else if (typeof ps === 'string') {
                            psText = ps;
                        }
                        itemPaperSizeMap[iid] = psText;
                    } catch (eItemLookup) {
                        log.error('SA Email [Step 3a.4]', 'Lookup paper_size for item ' + iid + ' failed: ' + (eItemLookup.message || eItemLookup));
                        itemPaperSizeMap[iid] = '';
                    }
                }
                log.debug('SA Email [Step 3a.4]', 'Paper Size map: ' + JSON.stringify(itemPaperSizeMap));
            } catch (eLookup) {
                throw new Error('getEmailData[item-lookup]: ' + (eLookup.message || eLookup));
            }

            // Merge paper_size + finalize items
