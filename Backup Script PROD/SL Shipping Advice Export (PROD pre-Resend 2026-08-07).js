    ...
    */
    ...
    function Page4_SubmitData(context, params) {
        ...
        try {
            ...
            let piData = libUtility.getLookupFields('salesorder', so_id_main, ['shipdate', 'custbody_exp_request__etd_date', 'custbody_ex_transfer_to_export', 'custbody_payment_terms', 'custbody_incoterms.custrecord_incl_fob_in_inv', 'custbody_ref_cash_discount']);
            ...
            for (var i = lineCountItem - 1; i >= 0; i--) {
                ...
                let itemtype = cRecord.getSublistValue({ sublistId: 'item', fieldId: 'itemtype', line: i });
                ...
                if (discountPct) {
                    ...
                    if (resultsSsSoSubtotalDiscount.discount.length > 0) {
                        resultsSsSoSubtotalDiscount.discount.forEach(function (disc) {
                            ...
                            amountDisc = libCode.toFixed2(amountDisc, 2); // round to 2 decimal places
                            ...
                            try {
                                ...
                                if (disc.pricelevel)
                                    cRecord.setCurrentSublistValue({ sublistId: 'item', fieldId: 'pricel', value: disc.pricelevel });
            ...
            if (piData.custbody_ref_cash_discount)
                cRecord.setValue({ fieldId: 'custbody_ref_cash_discount', value: piData.custbody_ref_cash_discount });
    ...
    function loadSOSubtotalDiscount(soId) {
        ...
        try {
            ...
            ssResults.forEach(function (row) {
                ...
                let fxamount = row.getValue({ "name": "fxamount", "label": "Amount (Foreign Currency)" });
                let rowData = {
                    internalid,
                    tranid,
                    item,
                    ...
                    taxcode,
                    location,
                    custcol_promotion_ordercode1,
                    custcol_promotion_orderamt1
                };
                if (type === 'Subtotal') {
                    result.subtotal = rowData;
                }
                else if (type === 'Discount') {
                    rowData.amount = Math.abs(fxamount) * -1; // Convert to negative for discount
            ...
            var totalSubtotal = Number(result.subtotal.fxamount) || 0;
            result.discount.forEach(function (disc) {
                ...
                disc.weightPct = Number(libCode.toFixed2((disc.weight * 100), 2));
            });
    ...
    const ShippingAdviceEmailModule = (function () {
        ...
        // CONFIG
        // ============================================================
        // Per-Sales-Channel routing via a shared "SA Email Setup" profile record.
        // One setup record = 1 Sender + 1 Recipients list, applied to one-or-more
        // Sales Channels via a multi-select field. See CONTEXT.md + docs/adr/0001.
        const SETUP_RECORD_TYPE = 'customrecord_sa_email_setup';
        const FLD_SETUP_CHANNELS = 'custrecord_sa_channels'; // multi-select -> customrecord_cseg_sale_channel
        const FLD_SETUP_SENDER = 'custrecord_sa_sender'; // select -> employee (email.send author)
        const FLD_SETUP_RECIPIENTS = 'custrecord_sa_email_recipients'; // free-text textarea, one email per line
        const SA_CHANNEL_FIELD = 'cseg_sale_channel'; // custom segment on SO header (mandatory)
        const MAX_RECIPIENTS = 20; // soft cap: warn (still send) if exceeded
        ...
        function sendConfirmedEmail(recid) {
            ...
            // 1) Skip if already sent (prevent duplicate) — cheap lookup first
            let soLookup;
            try {
            ...
            // 2) Load SO once. Read the Sales Channel via the Record API (NOT saved
            //    search) — custom-segment header fields are unreliable through search.
            let so;
            try {
                so = record.load({ type: record.Type.SALES_ORDER, id: recid });
                log.debug('SA Email [Step 2]', 'record.load OK');
            }
            catch (eLoad) {
                throw new Error('Step 2 (record.load SO) FAILED: ' + (eLoad.message || eLoad));
            }
            // 3) Resolve Sales Channel (mandatory on SA header)
            const channelId = so.getValue(SA_CHANNEL_FIELD);
            if (!channelId) {
                log.error('SA Email', 'SO ' + recid + ' has no ' + SA_CHANNEL_FIELD + ' value; cannot route. Skip (flag NOT set).');
                return;
            }
            log.debug('SA Email [Step 3]', 'channelId=' + channelId);
            // 4) Find the active SA Email Setup profile for this channel
            let setup;
            try {
                setup = getEmailSetupForChannel(channelId);
            }
            catch (e4) {
                throw new Error('Step 4 (getEmailSetupForChannel) FAILED: channel=' + channelId + ', ' + (e4.message || e4));
            }
            if (!setup) {
                log.error('SA Email', 'No active SA Email Setup for channel ' + channelId + ' (SO ' + recid + '). Skip (flag NOT set).');
                return;
            }
            // 5) Validate sender — author must be a valid, active employee with an email
            const senderId = setup.senderId;
            if (!senderId || !isValidSender(senderId)) {
                log.error('SA Email', 'Invalid/inactive sender employee (' + senderId + ') on setup ' + setup.setupId + ', channel ' + channelId + '. Skip (flag NOT set).');
                return;
            }
            // 6) Parse + validate recipients (one email per line; also tolerate , ; )
            const recipients = parseRecipients(setup.recipientsRaw);
            if (recipients.length === 0) {
                log.error('SA Email', 'No valid recipient emails on setup ' + setup.setupId + ', channel ' + channelId + '. Skip (flag NOT set).');
                return;
            }
            if (recipients.length > MAX_RECIPIENTS) {
                log.error('SA Email', 'Recipient count ' + recipients.length + ' exceeds soft cap ' + MAX_RECIPIENTS + ' on setup ' + setup.setupId + ' (channel ' + channelId + '). Sending to all valid anyway.');
            }
            log.debug('SA Email [Step 6]', 'Recipients: ' + recipients.length + ', sender=' + senderId);
            // 7) Build email content from the already-loaded record
            let data, subject, htmlBody;
            try {
                data = getEmailData(so);
                log.debug('SA Email [Step 7a]', 'getEmailData OK. tranid=' + data.tranid + ', items count=' + data.items.length);
            }
            catch (e7a) {
                throw new Error('Step 7a (getEmailData) FAILED: ' + (e7a.message || e7a));
            }
            try {
                subject = 'Shipping Advice Confirmed : ' + data.tranid;
                htmlBody = buildHtmlBody(data);
                log.debug('SA Email [Step 7b]', 'buildHtmlBody OK. body length=' + htmlBody.length);
            }
            catch (e7b) {
                throw new Error('Step 7b (buildHtmlBody) FAILED: ' + (e7b.message || e7b));
            }
            // 8) Send email
            try {
                email.send({
                ...
                log.debug('SA Email [Step 8]', 'email.send OK');
            }
            catch (e8) {
                throw new Error('Step 8 (email.send) FAILED: author=' + senderId + ', recipients=' + JSON.stringify(recipients) + ', error=' + (e8.message || e8));
            }
            // 9) Update flags (prevent duplicate + audit trail) — ONLY after a real send
            try {
                record.submitFields({
                    type: record.Type.SALES_ORDER,
                    id: recid,
                ...
                log.debug('SA Email [Step 9]', 'Update flags OK');
            }
            catch (e9) {
                throw new Error('Step 9 (submitFields email_sent flag) FAILED: ' + (e9.message || e9));
            }
            log.audit('SA Email', 'Sent OK. SO ID: ' + recid + ', channel=' + channelId + ', setup=' + setup.setupId + ', sender=' + senderId + ', recipients=' + recipients.length);
        }
        // ============================================================
        // PRIVATE : Find the active SA Email Setup profile for a channel
        // Filters the multi-select channel field with 'anyof'. If a channel
        // appears in more than one active setup (config error — NetSuite cannot
        // enforce uniqueness on a multi-select), use the FIRST and log.error.
        // See docs/adr/0001-sa-email-setup-multi-channel-profile.md.
        // ============================================================
        function getEmailSetupForChannel(channelId) {
            const found = [];
            search.create({
                type: SETUP_RECORD_TYPE,
                filters: [
                    [FLD_SETUP_CHANNELS, 'anyof', channelId],
                    'AND', ['isinactive', 'is', 'F']
                ],
                columns: [FLD_SETUP_SENDER, FLD_SETUP_RECIPIENTS, 'internalid']
            }).run().each(function (r) {
                found.push({
                    setupId: r.id,
                    senderId: r.getValue(FLD_SETUP_SENDER),
                    recipientsRaw: r.getValue(FLD_SETUP_RECIPIENTS)
                });
                return true;
            });
            if (found.length === 0)
                return null;
            if (found.length > 1) {
                log.error('SA Email', 'Channel ' + channelId + ' matches ' + found.length +
                    ' active SA Email Setup records (' + found.map(function (f) { return f.setupId; }).join(', ') +
                    '). Using first (' + found[0].setupId + '). Fix the duplicate config.');
            }
            return found[0];
        }
        // ============================================================
        // PRIVATE : Parse the free-text recipients field into a clean email array.
        // One email per line; also split on , and ; in case an admin pastes inline.
        // ============================================================
        function parseRecipients(raw) {
            return String(raw || '')
                .split(/[\r\n,;]+/)
                .map(function (s) { return s.trim(); })
                .filter(function (s) { return s && EMAIL_RE.test(s); });
        }
        // ============================================================
        // PRIVATE : Validate a sender employee is usable as email.send author
        // (exists, active, has an email address).
        // ============================================================
        function isValidSender(employeeId) {
            try {
                ...
                return !!f && f.isinactive === false && !!f.email;
            }
            catch (e) {
                log.error('SA Email [isValidSender]', 'Lookup employee ' + employeeId + ' failed: ' + (e.message || e));
                return false;
            }
        }
        // ============================================================
        // PRIVATE : Get header + line item data from a loaded Sales Order record
        // Uses the Record API (not Saved Search) to avoid saved-search column/join
        // compatibility issues with Custom Segments. Receives the already-loaded
        // record from sendConfirmedEmail so the SO is loaded only once.
        // @param {record.Record} so - loaded Sales Order record
        // ============================================================
        function getEmailData(so) {
